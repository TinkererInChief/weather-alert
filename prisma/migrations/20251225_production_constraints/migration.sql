...
CREATE INDEX IF NOT EXISTS idx_contacts_phone_active ON contacts(phone) WHERE active = true;
CREATE INDEX IF NOT EXISTS idx_contacts_notification_channels ON contacts USING GIN(notification_channels);
CREATE INDEX IF NOT EXISTS idx_contacts_coastal_resident ON contacts(is_coastal_resident) WHERE is_coastal_resident = true;

-- Indexes for SMS OTP table
CREATE INDEX IF NOT EXISTS idx_sms_otps_phone_active ON sms_otps(phone) WHERE consumed_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_sms_otps_expires ON sms_otps(expires) WHERE consumed_at IS NULL;

-- Indexes for earthquake events
CREATE INDEX IF NOT EXISTS idx_earthquake_events_occurred_at ON earthquake_events(occurred_at DESC);
CREATE INDEX IF NOT EXISTS idx_earthquake_events_magnitude ON earthquake_events(magnitude DESC);
CREATE INDEX IF NOT EXISTS idx_earthquake_events_location ON earthquake_events USING GIN(to_tsvector('english', location));
CREATE INDEX IF NOT EXISTS idx_earthquake_events_processed ON earthquake_events(processed, occurred_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_tsunami_alerts_created_at ON tsunami_alerts(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_tsunami_alerts_severity ON tsunami_alerts(severity_level DESC);
CREATE INDEX IF NOT EXISTS idx_tsunami_alerts_affected_zones ON tsunami_alerts USING GIN(affected_zones);

-- Indexes for alert jobs
CREATE INDEX IF NOT EXISTS idx_alert_jobs_status_priority ON alert_jobs(status, priority DESC, scheduled_for);
CREATE INDEX IF NOT EXISTS idx_alert_jobs_created_at ON alert_jobs(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_alert_jobs_type_severity ON alert_jobs(type, severity DESC);

-- Indexes for delivery logs (critical for monitoring)
CREATE INDEX IF NOT EXISTS idx_delivery_logs_alert_job_id ON delivery_logs(alert_job_id);
CREATE INDEX IF NOT EXISTS idx_delivery_logs_contact_id ON delivery_logs(contact_id);
CREATE INDEX IF NOT EXISTS idx_delivery_logs_status_channel ON delivery_logs(status, channel);
CREATE INDEX IF NOT EXISTS idx_delivery_logs_created_at ON delivery_logs(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_delivery_logs_sent_at ON delivery_logs(sent_at DESC) WHERE sent_at IS NOT NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_contacts_phone_unique_active 
